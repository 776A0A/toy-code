...
class Graph extends EventEmitter {
  ...
  init() {
    this.on({
      ...
      handler: () => {
        this.parentListeners.forEach(([type, handler]) =>
          this.offParent({ type, handler })
        )
      },
    })
  ...
  offParent({ type, handler }) {
    ...
    this.parentListeners = this.parentListeners.filter(
  ...
  attr(attrs = {}) {
    if (attrs.stickToParentCenter && !this.attrs.stickToParentCenter) {
      this.onParent({
        type: events.SIZE_CHANGED,
        handler: this.handleStickToCenter,
      })
  ...
  appendChild(...graphs) {
    ...
    graphs.forEach((graph) => {
      ...
      if (graph.attrs.stickToParentCenter) {
        graph.onParent({
          type: events.SIZE_CHANGED,
          handler: graph.handleStickToCenter,
        })
